...
class MediaHelper
{
    ...
    public static function getImageUrl(Model $model, string $field = 'image', bool $thumb = false)
    {
        ...
        if ($thumb) {
            return self::forceChangeUrlToPublic($model->$field->thumb);
        }

        return self::forceChangeUrlToPublic($model->$field->getFullUrl());
    }
     ...
     * @return string
     */
    public static function forceChangeUrlToPublic(string $link = "") : string
    {
        return str_replace(
            "/storage/",
            "/uploads/",
    ...
    {
        try {
//            dd(storage_path('tmp/uploads/' . $name));
            $model
                ->addMedia(storage_path('tmp/uploads/' . $name))
                ->toMediaCollection($field)->useDisk('local');
